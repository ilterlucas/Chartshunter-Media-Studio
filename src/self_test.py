...
import app
check('version', lambda: assert_true(app.APP_VERSION=='v43', app.APP_VERSION))
...
check('Brave cookie choice', lambda: assert_true(app.resolve_cookie_browser_choice('Brave (her linkte)')=='brave'))
check('smart cookie needs auth detector', lambda: assert_true(app.is_cookie_retry_error_text('HTTP Error 403: Forbidden - login required')))
check('smart cookie mode resolves no browser initially', lambda: assert_true(app.resolve_cookie_browser_choice(app.COOKIE_MODE_SMART) is None))
...
check('smart cookie default', lambda: assert_true('self.dl_cookies_browser = tk.StringVar(value=COOKIE_MODE_SMART)' in app_text))
check('smart cookie visible near link box', lambda: assert_true('cookie_quick = ttk.Frame(intro)' in app_text and 'Oturum / çerez' in app_text))
check('smart cookie retry path', lambda: assert_true('resolve_auto_cookie_browser()' in app_text and 'Oturum gerektiği algılandı' in app_text))
